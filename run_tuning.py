# Top-level script to run all 5 model tuning pipelines (zone + regional) sequentially
import os
import sys
import subprocess
...
TUNING_SCRIPTS = [
    ...
    ("Transformer - Regional",      "05_transformer/regional_level_tuning/regional_tuning.py"),
]

PROGRESS_FILE = os.path.join(BASE_DIR, "tuning_progress.txt")
...
def main():
    ...
    print(f"=== Wildfire Prediction — Full Tuning Pipeline ===")
    print(f"Found {len(completed)} already completed stages\n")

    for name, relative_path in TUNING_SCRIPTS:
    ...
    print(f"\n{'='*60}")
    print("All 10 tuning stages complete!")
    print(f"{'='*60}")
