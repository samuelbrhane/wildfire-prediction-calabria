# Regional-level hyperparameter tuning for Transformer
import os
import sys
import random
import numpy as np
...
from tuning_config import TRANSFORMER_SEARCH_SPACE as SEARCH_SPACE
...
results_csv_path = os.path.join(RESULTS_DIR, f"{GROUP_NAME}_transformer_results.csv")
...
for trial in range(completed_trials, NUM_TRIALS):
    ...
    try:
        ...
        (
            X_train, y_train, month_train, dow_train,
            X_val, y_val, month_val, dow_val,
            X_test, y_test, month_test, dow_test,
            df_test, scaler_y
        ) = preprocess_sequences_with_time(df.copy(), params)

        X_train_inputs = (X_train, month_train, dow_train)
        X_val_inputs = (X_val, month_val, dow_val)
        ...
        train_metrics = evaluate_model(
            model,
            [X_train, month_train, dow_train],
            y_train,
            scaler_y=scaler_y
        )
        val_metrics = evaluate_model(
            model,
            [X_val, month_val, dow_val],
            y_val,
            scaler_y=scaler_y
        )
...
print(f"\nTuning complete for regional Transformer model — results saved to {results_csv_path}")